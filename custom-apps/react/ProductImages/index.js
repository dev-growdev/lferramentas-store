...
const ProductImages = ({
  ...
  const excludeImage360Regex =
    prefix360 && prefix360.map(text => new RegExp(`^${text}`, 'i'))
  ...
  const checkIs360Image = image => {
    const is360 = excludeImage360Regex?.some(
      regex =>
        regex.test(image.imageLabel) ||
        regex.test(image.imageText) ||
        regex.test(
          image.imageUrl.split('/').filter(Boolean).pop().split('?')[0]
        )
    )

    if (is360) images360.push(image)
  ...
  const images = useMemo(() => {
    ...
    contentType,
    excludeImageRegexes,
    excludeImage360Regex,
    showImageLabel,
  ])

  const parsedImages360 =
    images360.length > 0
      ? [
          {
            type: 'image360',
            thumbUrl: '/arquivos/icon-360.svg',
            images: images360,
          },
        ]
      : []
  ...
  const slides = useMemo(() => {
    return showVideosFirst
      ? [...videos, ...images, ...parsedImages360]
      : [...parsedImages360, ...images, ...videos]
  ...
  if (displayMode === DISPLAY_MODE.LIST) {
    return (
      <div className={containerClass}>
        {images360.length > 0 ? (
          <Image360 images={images360} aspectRatio={aspectRatio} />
        ) : null}
