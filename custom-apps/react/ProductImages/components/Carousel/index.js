...
class Carousel extends Component {
  ...
  setInitialVariablesState() {
    ...
    slides.forEach(async (slide, i) => {
      ...
      if (slide.type === "image360") {
        this.setState(prevState => ({ ...prevState, has360Image: true }))
      }
    })
  }
  ...
  componentDidMount() {
    ...
    fetch('/arquivos/icon-360.svg')
      ...
      .then(svg => {
        ...
        if (svgElement) {
          svgElement.setAttribute('class', this?.props?.handles?.mobileButton360img);
          this.setState({ icon360svg: svgElement.outerHTML });
        }
  ...
  handleSlideChange = () => {
    this.setState(prevState => {
      ...
      const show360Button = this.state.has360Image && activeIndex !== 0;

      return { activeIndex, sliderChanged: true, show360Button }
    })
  }
    ...
    return (
      <div className={containerClasses} aria-hidden="true">
        {isThumbsVertical &&
          ...
          thumbnailSwiper}
        <div className={imageClasses}>
          {isMobile && this.state.show360Button
            ? (
              <span
                onClick={() => this.state.gallerySwiper.slideTo(0)}
                className={handles.mobileButton360}
                style={{ position: "absolute", zIndex: 10, top: '30px', left: 0 }}
                dangerouslySetInnerHTML={{ __html: this.state.icon360svg }}
              />
            )
            : null
          }
          {!this.state.thumbSwiper?.destroyed && (
            <Swiper
              ...
              noSwipingClass={handles.noSwiping}
              {...this.galleryParams}
            >
              {slides.map((slide, i) => (
                <SwiperSlide
                  key={`slider-${i}`}
                  noSwiping={slide.type === "image360" ? true : false}
                  className={`${handles.productImagesGallerySlide} ${slide.type === "image360" ? handles.noSwiping : ''} swiper-slide center-all`}
                >
