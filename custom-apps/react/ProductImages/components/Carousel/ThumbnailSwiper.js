...
const Thumbnail = props => {
  const { alt, isVideo, is360, thumbUrl, handles, aspectRatio = 'auto', thumbnailMaxHeight = 'unset' } = props

  return (
    <>
      <figure
        className={`${applyModifiers(handles.figure, isVideo ? 'video' : is360 ? '360' : '')}`}
      ...
      >
        <img
          className={`${applyModifiers(
            handles.thumbImg,
            isVideo ? 'video' : is360 ? '360' : '',
          )} w-100 h-auto db`}
          itemProp="thumbnail"
          alt={alt}
          src={imageUrl(thumbUrl, THUMB_SIZE, THUMB_MAX_SIZE, aspectRatio)}
          style={{ aspectRatio, maxWidth: `${THUMB_MAX_SIZE}px`, maxHeight: is360 ? '100%' : `${thumbnailMaxHeight}px`, height: is360 ? `${thumbnailMaxHeight}px` : undefined }}
          width={THUMB_SIZE}
        />
      </figure>
      <div
...
const ThumbnailSwiper = props => {
  ...
  const thumbClassName = classNames(
    `${handles.carouselGaleryThumbs} dn h-auto`,
    {
      ...
      'w-auto bottom-0 top-0 absolute': isThumbsVertical,
      'left-0':
        ...
        {slides.map((slide, i) => {
          return (
            <SwiperSlide
              ...
              style={{
                height: isThumbsVertical ? 'auto' : thumbnailMaxHeight || '115px',
                width: 'auto',
                aspectRatio: thumbnailAspectRatio || 'auto',
