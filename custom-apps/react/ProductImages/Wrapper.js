...
import QUERY_PRODUCT from './graphql/product.graphql'
...
const ProductImagesWrapper = props => {
  ...
  const { data: productGraphql, error } = useQuery(QUERY_PRODUCT, {
    variables: {
      identifier: {
        field: 'id',
        value: runtime?.route?.params?.id,
      },
    },
    ssr: true,
  ...
  const assetsSpecificationGroup =
    product?.specificationGroups?.find(
      specGroup => specGroup.name === 'Configurações'
    ) ||
    productGraphql?.product?.specificationGroups?.find(
      specGroup => specGroup.name === 'Configurações'
    )

  const prefix360 = assetsSpecificationGroup?.specifications?.find(
    spec => spec.name === 'prefixo 360'
  )?.values
