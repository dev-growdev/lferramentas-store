...
const HelperProductSKU = () => {
  ...
  const removeDuplicatedProductName = (productNames: HTMLCollection) => {
    ...
    if (
      productNames[0].textContent?.replace(/\s/g, '').toLocaleLowerCase() ==
      productNames[1].textContent?.replace(/\s/g, '').toLocaleLowerCase()
    ) {
  ...
  React.useEffect(() => {
    const productNames = window.document?.getElementsByClassName(
